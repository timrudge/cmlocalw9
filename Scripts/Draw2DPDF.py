...
class CellModellerPDFGenerator(Canvas):
    ...
    def draw_capsule(self, p, d, l, r, fill_color, stroke_color):
        ...
        self.setLineWidth(0.005*units.cm)
        self.setLineWidth(0.005*units.cm)
...
# Define a pdf generator class that simulates fluorescence images
class FluoGenerator(CellModellerPDFGenerator):
    def calc_cell_colors(self, state):
        # Generate Color objects from cellState, fill=stroke
        (r,g,b) = state.color
        # Return value is tuple of colors, (fill, stroke)
        fcol = Color(r,g,b,alpha=1.0)
        scol = Color(0,0,0,alpha=1)
        return [fcol,scol]
...
def main():
    ...
    for infn in infns:
        ...
        pdf = FluoGenerator(outfn, data, bg_color)
